...
# Whole-volume 2D SwinIR inference over every degraded input in the held-out
# nii_test split, writing the restored volumes into a mirror tree at
...
#   sbatch experiments/01-model-comparison/enhance_swinir.sh raw       (-> fully sampled GT)
#   sbatch experiments/01-model-comparison/enhance_swinir.sh denoised  (-> md denoised+biascorr GT)
#
...
# evaluate_baseline_nii_test.sh swinir <TARGET> (submit_baselines.sh chains
...
TARGET=${1:-raw}
case "$TARGET" in
    raw)       DEFAULT_CKPT=/vast/tibrahim/jil202/autoflow/checkpoints_swinir/swinir_2d_raw_brain_all_090726_best_md.pt ;;
    denoised|md) TARGET=denoised
               DEFAULT_CKPT=/vast/tibrahim/jil202/autoflow/checkpoints_swinir/swinir_2d_denoised_brain_all_090726_best_md.pt ;;
...
OUT_ROOT=$EXP_DIR/enhanced_nii_test_swinir_${TARGET}${SUFFIX}
...
for in_path in "${INPUTS[@]}"; do
    ...
    echo "=== $rel ==="
    python "$EXP_DIR/enhance_swinir.py" \
        --input_path "$in_path" \
